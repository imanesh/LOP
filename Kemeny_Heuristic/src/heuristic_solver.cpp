...
//	cout << "Initial solution creation started" << endl;
	...
	for(int i =0; i < R.Candidates(); i++)
		for(int j =0; j < R.Candidates(); j++)
			s.sol_vector[i][j] = cplex.getValue(x[i][j]);
	env.end();
	//cout << "creating initial solution ends." << endl;
	return s;
}
...
vector<int> HeurSolver::Preprocessing(int grasp_cons_id){
	//cout << "preprocessing start .." << endl;
	vector<vector<int> > subranking;
	vector<int> partial_order;	
	vector<int> temp_2;
	int gsp_cons_method = 2;
	float gsp_alpha = 0.4;
	IloEnv env;
	vector<int> init_ordering; 
	if (grasp_cons_id == 0){
		//call LP
		solution lp = SolveLP();	
	//	cout << "LP solution is : " << lp.obj_value << endl;
		solution initial = CreateInitialSolution(lp);
		init_ordering = MatrixToOrdering(initial.sol_vector, R.Candidates());
	}
	else {
			Grasp gsp(R);
			gsp.grasp(init_ordering,grasp_cons_id, gsp_alpha, 500);
	}
			
	if (R.Candidates() <= 100 ){	
		//cout << "init in mip value is " << OrderingValue(init_ordering) << endl;
		// end of grasp test 
		int mid_point = floor(R.Candidates()/2);
		//cout << "midpoint is " << mid_point << endl;
		//cout << "init_order size " << init_ordering.size() << endl;
		...
		//cout << "temp_1 is = " << endl << temp_1 << endl;
		...
		//cout << "temp_2 is after = " << endl << temp_2 << endl;
		...
		//cout << "temp_3 is = " << endl << temp_3 << endl;
	
		for(int i = 0; i < temp_3.size(); i++)
			temp_2[i+one_fifth_point] = temp_3[i];
		temp_3.clear();	
		//cout << "temp_2 is = " << endl << temp_2 << endl;
		...
		//vector<int> init_ordering;
		//Grasp gsp(R);
		//gsp.grasp(init_ordering,gsp_cons_method, gsp_alpha,  500);
		// end of grasp test 
...
vector<int> HeurSolver::Preprocessing(){
	...
	if (R.Candidates() <= 100 ){
		...
		/* comment out to feed the grasp solution instead of LProunding heuristuc 
		solution lp = SolveLP();	
		cout << "LP solution is : " << lp.obj_value << endl;
		solution initial = CreateInitialSolution(lp);
		vector<int> init_ordering = MatrixToOrdering(initial.sol_vector, R.Candidates());
		...
		*/
		...
		vector<int> init_ordering;
		Grasp gsp(R);
		gsp.grasp(init_ordering,gsp_cons_method, gsp_alpha,  300);
		cout << "init ordering size " << init_ordering.size() << endl;
		cout << "init in mip value is " << OrderingValue(init_ordering) << endl;
		// end of grasp test 
		int mid_point = floor(R.Candidates()/2);
		cout << "midpoint is " << mid_point << endl;
...
void HeurSolver::Test(){
	...
	cout << "order value = " << OrderingValue(temp_2);
	vector<int> test(100, 0);
	cout << "test befor = " << test << endl;
	for(int i = 0; i < 100; ++i)
		++test[temp_2[i]];
...
void HeurSolver::MIP_HeuristicTest(){
	//cout << endl << "### The Iterative MIP (with intial solution) is starting....." << endl;
	time_t start, end;
	time(&start);
	...
	while(iteration++ < 5000)
	{
		...
		int indx_begin = rand()%(R.Candidates() - seq_size);
		//////////////////////////
		if (random_index.size() != R.Candidates() - seq_size)
			while(random_index.find(indx_begin) != random_index.end())
				indx_begin = rand()%(R.Candidates() - seq_size);
		...
		if(value > max_value){
			...
			improve_counter++;
			// to keep what random index does not lead to improvemetn and not use them again
			// befor any seen improvemnt, need to be more explored
			random_index.insert(indx_begin);
			cout << "$$$$the couneter improvement is = " << improve_counter << endl;
		}
		if (improve_counter >  100){
			//	k++;//	p++;
	...
	*/
	...
	//last local search step
//	Grasp gsp(R);
//	vector<int> final_order = gsp.local_search_improvement(temp_2);
//	cout << "final afetr local serach is = " << OrderingValue(final_order) << endl;
//	cout << "final order is " << final_order << endl;	
	...
	vector<int> test(R.Candidates(),0);
	cout << "test befor = " << test << endl;
	for(int i = 0; i < R.Candidates(); ++i)
		++test[temp_2[i]];
	cout << "test after "<< test << endl;
...
int HeurSolver::MIP_Heuristic(int grasp_cons_id){
	srand(time(NULL));
	time_t start, end;
	time(&start);
	multiset<TS> tabu_table;
	...
	vector<int> temp_2 = Preprocessing(grasp_cons_id);
	vector<int> partial_order;	
	vector<vector<int> > subranking ;	
	//local search second phase
	int iteration = 0;								//number of local search iteration
...
//		cout << endl << " and seq_size is : " << seq_size << endl;
		...
		int indx_begin = rand()%(R.Candidates() - seq_size);
		//////////////////////////
		//	cout << "available size is " << R.Candidates() - seq_size << endl;	
		//	cout << "the size of set rand index is " << random_index.size() << endl;	
		if (random_index.size() <  R.Candidates() - seq_size)
			while(random_index.find(indx_begin) != random_index.end())
				indx_begin = rand()%(R.Candidates() - seq_size);
		//	cout << endl << "**** rand inedex is " << indx_begin << " **** " << endl;
...
//		cout << "$$$$$ value = " << value << " and max_value is = " << max_value << endl;
		...
		if (improve_counter >  100){
			//	k++;//	p++;
			gsp.local_search_improvement(temp_2);
			int temp_value = OrderingValue(temp_2);
...
//		cout << endl << " The best solution found up to iteration " << iteration << " is "  << max_value << endl;
	...
	//last local search step
	gsp.local_search_improvement(temp_2);
	int final_value = OrderingValue(temp_2);
	//	cout << "final afetr local serach is = " << OrderingValue(temp_2) << endl;
//	cout << "final order is " << temp_2 << endl;	
...
void HeurSolver::MIP_Heuristic_copy(){
	cout << endl << "### The Iterative MIP (with intial solution) is starting....." << endl;
	srand (time(0));
	time_t start, end;
	time(&start);
...
//helper function
void initiate_rounded_pairs(vector<pair<int,int> >& rounded_pairs)
{

	rounded_pairs.push_back(make_pair(0,1));
	rounded_pairs.push_back(make_pair(0,3));
	rounded_pairs.push_back(make_pair(4,0));
	rounded_pairs.push_back(make_pair(6,0));
	rounded_pairs.push_back(make_pair(7,0));
	rounded_pairs.push_back(make_pair(8,0));
	rounded_pairs.push_back(make_pair(10,0));
	rounded_pairs.push_back(make_pair(1,2));
	rounded_pairs.push_back(make_pair(3,1));
	rounded_pairs.push_back(make_pair(4,1));
	rounded_pairs.push_back(make_pair(1,5));
	rounded_pairs.push_back(make_pair(7,1));
	rounded_pairs.push_back(make_pair(8,1));
	rounded_pairs.push_back(make_pair(9,1));
	rounded_pairs.push_back(make_pair(4,2));
	rounded_pairs.push_back(make_pair(8,2));
	rounded_pairs.push_back(make_pair(9,2));
	rounded_pairs.push_back(make_pair(10,2));
	rounded_pairs.push_back(make_pair(9,3));
	rounded_pairs.push_back(make_pair(4,5));
	rounded_pairs.push_back(make_pair(4,7));
	rounded_pairs.push_back(make_pair(4,8));
	rounded_pairs.push_back(make_pair(5,6));
	rounded_pairs.push_back(make_pair(7,5));
	rounded_pairs.push_back(make_pair(7,8));
	rounded_pairs.push_back(make_pair(7,10));
	rounded_pairs.push_back(make_pair(8,9));
	//rounded_pairs.push_back(make_pair(0,5));
//	rounded_pairs.push_back(make_pair(6,1));
//	rounded_pairs.push_back(make_pair(5,0));
}

// This is a heusritic that works with the history matrix to fix and relax
// pairs in the integr model
vector<int> HeurSolver::IterativeRelaxing()
{
	int y =0;
	for (int i =0; i < R.Candidates(); ++i)
		for(int j =0; j < R.Candidates();++j)
	...
	cout << "iterative relaxing is starting ..."  << endl;
	Grasp gsp(R);	
	int grasp_iteration = 500;
	int rounding_treshold = 300;
	int gsp_const_method = 1;
	float gsp_alpha = 0.8;
	vector<vector<int> >history = gsp.grasp(gsp_const_method, gsp_alpha,grasp_iteration);	
	vector<int> init_order;
	Graph G(R.Candidates(), true);
	try{
		// prepare the set of rounded to 1 pairs
		vector<pair<int,int> > rounded_pairs;
		for (int i = 0; i < R.Candidates(); ++i)
			for(int j =i+1; j < R.Candidates();++j)
				if(history[i][j] >= rounding_treshold)
					if(!G.isPath(j,i)){
						G.insert(Edge(i,j));
						rounded_pairs.push_back(make_pair(i,j));
					}
				else if (history[j][i] >= rounding_treshold)
					if(!G.isPath(i,j)){
						G.insert(Edge(j,i));
						rounded_pairs.push_back(make_pair(j,i));
					}

		/*vector<pair<int,int> > y;
		initiate_rounded_pairs(y);
		vector<pair<int,int> >::iterator pt;
		for(pt = y.begin(); pt != y.end(); ++pt)
			if(!G.isPath(pt->first, pt->second))
				rounded_pairs.push_back(make_pair(pt->first,pt->second));
		*/		
		
		cout << "rounded pair is =  " << rounded_pairs << endl; 
		//cretae the model
		//cout << "****** init_order value is " << OrderingValue(init_order) << endl;
		solution s(R.Candidates());
		IloEnv env;
		...
		//fix varibles based on rounded_pair set	
		IloConstraintArray cons_array(env);
		vector<IloConstraint> cons_list;
		int nCuts =0;		
		vector<pair<int,int> >::iterator it;
		for(it = rounded_pairs.begin(); it != rounded_pairs.end(); ++it){
			IloConstraint cons = x[it->first][it->second] == 1; 
			model.add(cons);
			cons_array.add(cons);
		...
		cout << "nrows and ncols after adding  = " << nrows << " , " << ncols << endl;
		...
		s.time = difftime(end, start);
		s.obj_value = cplex.getObjValue();
		cout << "The initial sol is " << cplex.getObjValue() <<  endl;
		for(int i =0; i < R.Candidates(); i++)
			for(int j =0; j < R.Candidates(); j++)
				s.sol_vector[i][j] = cplex.getValue(x[i][j]);
		init_order = MatrixToOrdering(s.sol_vector, R.Candidates());	
		//prepare vars to feed to model as an initial solution
		IloNumArray stVal = OrderToNumArray(env,init_order);
		//cplex.addMIPStart(stVar, stVal, IloCplex::MIPStartAuto,  "secondMIPStart");
	
		/*
		list<IloConstraint>::iterator lt;
		for(lt = cons_list.begin(); lt != cons_list.end(); ++lt)
			model.remove(*lt);
		*/
	   //for(int i =0; i < nCuts ; ++i)
		...
		cout << "nrows and ncols after 2  adding  = " << nrows << " , " << ncols << endl;
		if ( !cplex.solve() ) {
			env.error() << "Failed to optimize LP." << endl;
			throw (-1);
		}
		//fill the solution
		s.obj_value = cplex.getObjValue();
		cout << "The final sol is " << cplex.getObjValue() <<  endl;	
		
		double relaxing_portion = 0.2;
		cout << "****** init_order value is " << OrderingValue(init_order) << endl;
		int iteration = 10;
		int itr_idx =0;
		while (itr_idx < iteration && cons_list.size() > 0){
			cout << "itr = " << itr_idx << endl;	
			int n_relax_pair = floor(cons_list.size() * relaxing_portion); // number of constraint that must be deleted
			cout << "number of relax pair in this iteration = " << n_relax_pair << endl;
			cout << "cons_list size is " << cons_list.size() << endl;
			for(int i = n_relax_pair; i >= 0; --i){
				//int d = rand() % cons_list.size();
				//assert( d <= cons_list.size());
				IloConstraint d = cons_list.back();
				model.remove(d); 	//remove the constraint from the model
				cons_list.pop_back();
			}
			if ( !cplex.solve() ) {
				env.error() << "Failed to optimize LP." << endl;
			...
			env.out() << "obj value in this iterarion is = " << cplex.getObjValue() << endl;
			for(int i =0; i < R.Candidates(); i++)
				for(int j =0; j < R.Candidates(); j++)
					s.sol_vector[i][j] = cplex.getValue(x[i][j]);
